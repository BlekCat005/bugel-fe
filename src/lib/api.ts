...
export type KontakResponse = KontakPayload & {
	id: string;
	tanggal: string;
};

export function kirimPesanKontak(payload: KontakPayload) {
	return request<KontakResponse>("/api/kontak", {
		method: "POST",
		body: JSON.stringify(payload),
	});
}
